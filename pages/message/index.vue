...
<template>
  <view class="page-container">
    <view class="message-tabbar">
      <text v-for="(tab, index) in tabsData" :key="tab.contentType" :class="{ active: tabIndex === index }" @click="onTabChange(index)" class="tab">{{ tab.text }}</text>
    </view>
      ...
      <view class="scroll-view-wrapper">
        ...
        <uni-card v-for="item in tabsData[0].items" :key="item.id" :border="false" :is-shadow="false">
          ...
          <view class="extra">
            ...
            <navigator hover-class="none" class="link" :url="`/subpkg_message/content/index?id=${item.id}`">查看详情</navigator>
          </view>
