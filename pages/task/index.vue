  ...
  <view class="page-container">
        ...
        >
          ...
          >
            <view class="header">
              ...
              <text
                v-if="item.actualArrivalTime > item.planArrivalTime"
                class="status"
                >已延迟</text
              >
            </view>
      ...
      <view class="scroll-view-wrapper">
        ...
        >
          ...
          <view class="footer">
            ...
            <navigator
              v-if="item.status === 4"
              :url="`/subpkg_task/record/index?id=${item.id}&startTime=${item.actualDepartureTime}`"
              hover-class="none"
              class="action"
            >
              回车登记
    ...
    <view v-show="tabIndex === 2" class="task-search">
      <view class="search-bar">
        ...
        <input
          v-model="tabsData[2].transportTaskId"
          class="input"
          type="text"
          placeholder="输入任务编号"
        />
      ...
      <view class="filter-bar">
        <picker
          @change="onPickerChange($event, 'startTime')"
          class="picker"
          mode="date"
          >{{ tabsData[2].startTime || '开始时间' }}</picker
        >
        <text class="text">至</text>
        <picker
          @change="onPickerChange($event, 'endTime')"
          class="picker"
          mode="date"
          >{{ tabsData[2].endTime || '结束时间' }}</picker
        >
    ...
    >
      <view class="scroll-view-wrapper">
        <view
          v-for="item in tabsData[2].items"
          :key="item.id"
          class="task-card"
        >
          <navigator
